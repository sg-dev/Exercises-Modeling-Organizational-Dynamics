...
class Node(Agent):
    def __init__(self, model, load, capacity):
    ...
    def inward(self):
        ...
        for neighbor in self.inneighbors():
            if neighbor.failed or neighbor.failing:
                failed_neighbors += 1
        self._next_load = (failed_neighbors / self.indegree) if self.indegree > 0 else 0
    ...
    def step(self):
        ...
        if model_type == "constant":
            if load_type == "in":
                ...
                self.outward()
        elif model_type in ["load", "overload"]:
            if load_type == "llsc":
                self.llsc()
            elif load_type == "llss":
                self.llss()
