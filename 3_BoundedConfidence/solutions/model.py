...
class BoundedConfidenceModel(Model):
    def __init__(self, num_agents, mu, epsilon, model_type, max_steps=500):
    ...
    def step(self):
        ...
        if self.model_type == "pairwise":
            # Select two distinct agents at random.
            i, j = np.random.choice(self.agents, 2, replace=False)
            if abs(i.opinion - j.opinion) < self.epsilon:
                i._next_opinion = i.opinion + self.mu * (j.opinion - i.opinion)
                j._next_opinion = j.opinion + self.mu * (i.opinion - j.opinion)
                i.opinion = i._next_opinion
                j.opinion = j._next_opinion
            self.step_count += 1
        else:
